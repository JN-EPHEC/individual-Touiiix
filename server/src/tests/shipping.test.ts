    ...
    describe('1. Catalog and boudaries', () => {
        const validCases = [
            ...
            test.each(validCases)(
                'Distance %i km, Poids %i kg (%s) -> Doit couter %i€',
                (distance, poids, type, resultatAttendu) => {
                    expect(calculateShipping(distance as number, poids as number, type as "standard" | "express")).toBe(resultatAttendu);
                }
            )
        const errorCases = [
        ...
        test.each(errorCases)(
            'Entrée invalide (%i, %i) doit lever une erreur',
            (distance , poids, type, ) => {
                expect(() => {
                    calculateShipping(distance as number, poids as number, type as "standard" | "express");
                }).toThrow();
            }
        )
    })
